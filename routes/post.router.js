...
postRouter.get("/date", async (req, res, next) => {
  try {
    const { body } = req;
...
postRouter.get("/:id", verifyToken, async (req, res, next) => {
  try {
    const { params } = req;
    const { id } = params;
    console.log(id);
...
postRouter.put("/:id", verifyToken, async (req, res, next) => {
  try {
...
postRouter.delete("/:id", verifyToken, async (req, res, next) => {
  try {
    const { params } = req;
    const { id } = params;
    console.log(id);
